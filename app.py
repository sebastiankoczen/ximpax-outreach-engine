    ...
    if isinstance(sources, list):
        for s in sources:
            if isinstance(s, dict):
                ...
                uri = s.get("uri", "")
                date = s.get("published_at") or s.get("date") or s.get("article_date") or ""
...
with st.sidebar:
    ...
    if gemini_key:
        st.success("Gemini API configured securely on the server")
    else:
...
if "result" in st.session_state:
    ...
    with st.expander("🔍 API Debug"):
        st.text(f"Key loaded: {'YES' if gemini_key else 'NO'}")
