...
def _fetch_article_date(uri, timeout=3):
    """Try to extract a publication date from common HTML meta tags / JSON-LD."""
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            # Read only the first 12 KB — dates are always in <head>
            html = resp.read(12_000).decode("utf-8", errors="ignore")

        patterns = [
            # OpenGraph / standard meta
            r'<meta[^>]+property=["\']article:published_time["\'][^>]+content=["\']([^"\']{10})',
            r'<meta[^>]+name=["\'](?:date|pubdate|publish[_-]?date|publication[_-]?date)["\'][^>]+content=["\']([^"\']{10})',
            # JSON-LD
            r'"datePublished"\s*:\s*"([^"]{10})',
            r'"publishedAt"\s*:\s*"([^"]{10})',
            r'"dateCreated"\s*:\s*"([^"]{10})',
        ]
        for pat in patterns:
            m = re.search(pat, html, re.IGNORECASE)
            if m:
                raw = m.group(1)[:10]          # keep only YYYY-MM-DD
...
def _extract_grounding_sources(resp):
    ...
    # Enrich each source with a publication date (best-effort, never blocks)
    for s in unique:
